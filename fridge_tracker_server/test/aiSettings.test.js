...
test("user AI settings encrypt API keys and remain isolated per user", () => {
  const db = createTestDatabase();
  const settings = createAiSettingsService(db, "test-encryption-secret");
  const saved = settings.saveSettings(1, {
  ...
  assert.equal(db.prepare("SELECT api_key_encrypted FROM user_ai_settings WHERE user_id = 1").get().api_key_encrypted.includes("sk-user"), false);
...
test("updating model settings preserves a blank API key and validates base URLs", () => {
  const db = createTestDatabase();
  const settings = createAiSettingsService(db, "test-encryption-secret");
  settings.saveSettings(1, { openaiApiKey: "secret-key", openaiModel: "old", openaiBaseUrl: "https://api.openai.com/v1" });
  settings.saveSettings(1, { openaiApiKey: "", openaiModel: "new", openaiBaseUrl: "http://127.0.0.1:11434/v1" });
