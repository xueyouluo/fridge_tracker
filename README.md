...
- 在手机 H5 中记录食材、购买日期、保鲜天数、数量和分类。
- 自动计算到期状态，优先显示已过期和即将到期的食材。
- ESP32 定时联网获取新画面，无变化时不重复刷新，随后进入深度睡眠。
- 支持竖屏和横屏，以及 4.2 寸三色、3.97 / 7.5 寸四色墨水屏。
- 可选使用内置文字助手或 MCP，让 Agent 查询和管理自己的食材。
- 不同家庭的食材和设备相互隔离；管理员可提供系统 Agent，用户也可配置仅自己使用的 API Key。
...
- OpenAI-compatible 模型 API Key，可由服务管理员统一提供，也可由用户配置自己的 Key；显示食材不需要。
...
打开 [公共托管服务](https://fridge.followllm.online) 注册账号，添加几条食材，
...
在 H5 添加或修改一条食材，等待设备下次检查，或重新上电触发检查。成功时应看到：

- “设备”页面出现该设备，并显示最近同步时间。
- 串口日志先显示 Wi-Fi 地址和注册成功，再显示 `Stored frame bytes`。
- 4.2 寸三色屏的帧大小为 `30000` 字节；两款四色屏为 `96000` 字节。
- 屏幕显示新食材，随后日志显示屏幕休眠和 ESP32 深度睡眠。
- 食材没有变化时服务返回 `304`，设备不重复刷新，这是正常的省电行为。
...
基础食材管理和墨水屏同步不需要配置模型。管理员可在“用户 → 系统 Agent”中提供统一的
