...
- 配网页面可设置 5–1440 分钟的服务端检查间隔，默认 30 分钟。
...
`http://192.168.4.1`，即可修改 Wi-Fi、服务地址和检查间隔。无人操作时，热点
...
`ETag` 返回 `304`，固件不会重复下载 96,000 字节帧文件，但会从 FFat 读取已
缓存的画面并重新执行 `writeNative()` 和 `refresh(false)`。串口会显示
